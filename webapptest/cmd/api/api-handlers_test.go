...
func Test_app_userHandlers(t *testing.T) {
	var tests = []struct {
		...
		{"deleteUser bad URL param", "DELETE", "", "Z", app.deleteUser, http.StatusBadRequest},
		...
		},
		{
			"insertUser valid",
			"PUT",
			`{"first_name": "Jack", "last_name":"wook", "email": "wook@example.com"}`,
			"",
			app.insertUser,
			http.StatusNoContent,
		},
		{
			"insertUser invalid",
			"PUT",
			`{"foo": "bar", "first_name": "Jack", "last_name":"wook", "email": "wook@example.com"}`,
			"",
			app.insertUser,
			http.StatusBadRequest,
		},
	...
	for _, e := range tests {
		...
		if rr.Code != e.wantStatusCode {
			t.Errorf("%s: wrong status return want %d but got %d", e.name, rr.Code, e.wantStatusCode)
		}
	}
}
