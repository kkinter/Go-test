...
func TestPostgresDBRepoInsertUser(t *testing.T) {
	...
	if id != 1 {
		t.Errorf("insert user returned wrong id: want 1, but got %d", id)
	}
}
...
func TestPostgresDBRepoAllUsers(t *testing.T) {
	...
	if len(users) != 1 {
		t.Errorf("all users reports wrong size: want 1, but got %d", len(users))
	}
	...
	if len(users) != 2 {
		t.Errorf("all users reports wrong size after insert: want 2, but got %d", len(users))
	}
}
...
func TestPostgresDBRepoGetUser(t *testing.T) {
	...
	if user.Email != "admin@example.com" {
		t.Errorf("wrong email returned by GetUser: want admin@example.com , but got %s", user.Email)
	}

	_, err = testRepo.GetUser(3)
	if err == nil {
		t.Error("no error reported whne getting non existent user by id")
	}
}
...
func TestPostgresDBRepoGetUserByEmail(t *testing.T) {
	...
	if user.ID != 2 {
		t.Errorf("wrong ID returned by GetUserByEmail: want 2 , but got %d", user.ID)
	}
}
...
func TestPostgresDBRepoUpdateUser(t *testing.T) {
	...
	if user.FirstName != "Jane" || user.Email != "jane@smith.com" {
		t.Errorf("want updated record to have first name Jane and email jane@smith.com, but got %s %s", user.FirstName, user.Email)
	}
}
...
func TestPostgresDBRepoDeleteUser(t *testing.T) {
	...
	if err == nil {
		t.Errorf("want deleted but not deleted")
	}
}
...
func TestPostgresDBRepoResetPassword(t *testing.T) {
	...
	if !matches {
		t.Errorf("password shouldn't match")
	}
}

func TestPostgreDBRepoInsertUserImage(t *testing.T) {
	...
	if newID != 1 {
		t.Error("got wrong ID for image: want 1 got", newID)
	}
